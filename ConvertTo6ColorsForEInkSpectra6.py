...
def process_image(image_file, dither, display_mode, width, height, scale, brightness, contrast, saturation):
    try:
        ...
        dither_label = 'ATK' if dither == 1 else 'FS' if dither == 3 else ''
        ...
        if width is not None and height is not None:
            ...
            target_width = max(1, int(img_width * scale))
            target_height = max(1, int(img_height * scale))
        ...
        if display_mode == 'scale':
            ...
            if img_width / img_height >= target_width / target_height:
                ...
                delta_width = int(img_height * target_width /
                ...
                delta_height = int(img_width * target_height /
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
        futures = {executor.submit(worker_fn, f): f for f in all_image_files}
